...
function getRecentConsoleLogs(maxLines = 30) {
	try {
		...
		const mod =
			globalThis.__consoleLogBuffer ||
			require?.cache?.[require?.resolve?.("./consoleLogBuffer.js")];
...
export function initCrashLogger() {
	...
	global.__logCrash = (
		type = "manual",
		error = new Error("Manual crash log"),
		extra = {},
	) => {
	...
	process.on("beforeExit", (code) => {
		if (code !== 0) {
			try {
				writeCrashReport(
					"beforeExit",
					new Error(`Process exiting with code ${code}`),
					{ exitCode: code },
				);
			} catch {}
		}
	...
	console.log(
		`[crashLogger] Global handlers registered. Crashes will be logged to: ${CRASH_LOG_PATH}`,
	);
}
